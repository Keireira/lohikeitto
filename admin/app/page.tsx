		...
		<>
				...
				actions={
					<div className="flex items-center gap-3">
						<div className="flex items-center gap-4 text-xs text-muted-fg">
							<span>
								<strong className="text-foreground">{services.length}</strong> services
							</span>
							<span>
								<strong className="text-foreground">{verified}</strong> verified
							</span>
							<span>
								<strong className="text-foreground">{catCount}</strong> categories
							</span>
						</div>
			...
			<div className="p-8">
				<ServicesTable
					...
					s3Logos={s3Objects
						.filter((o) => o.key.startsWith('logos/') && !o.key.endsWith('/') && o.size > 0)
						.map((o) => o.key.replace('logos/', '').replace(/\.[^.]+$/, ''))}
				/>
			</div>
		</>
	);
