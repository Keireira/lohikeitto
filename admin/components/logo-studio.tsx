	...
	const handleLocal = async (file: File) => {
		...
		setLogos((p) => ({
			...
			local: {
				...
				width: dims.w,
				height: dims.h,
				format: file.type.split('/').pop() ?? file.name.split('.').pop() ?? '?'
			}
		}));
		setActive('local');
	};
										...
										{entry ? (
											...
											<div className="size-12 flex items-center justify-center bg-muted/40">
												{loading.has(src) ? (
													...
													<span className="text-[8px] text-muted-fg/30 uppercase tracking-wider">
														{src === 'logodev' ? 'L.D' : src.slice(0, 3)}
													</span>
												)}
											</div>
										)}
						...
						<div className="flex-1 overflow-y-auto px-5 py-4 space-y-1.5">
							...
							>
								{loading.has('brandfetch') ? (
									<span className="flex items-center gap-2">
										<span className="size-3 rounded-full border-2 border-accent/30 border-t-accent animate-spin" />{' '}
										Fetching...
									</span>
								) : (
									<span className="flex items-center justify-between">
										Brandfetch{logos.brandfetch && <span className="text-success text-xs">{'✓'}</span>}
									</span>
								)}
							</button>
							...
							>
								{loading.has('logodev') ? (
									...
									<span className="flex items-center justify-between">
										logo.dev{logos.logodev && <span className="text-success text-xs">{'✓'}</span>}
									</span>
								)}
							</button>
							...
							>
								<span className="flex items-center justify-between">
									Upload file{logos.local && <span className="text-success text-xs">{'✓'}</span>}
								</span>
							</button>
							{logos.local && (
								<button
